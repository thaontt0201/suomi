...
import { speakingTaskKindLabel } from "@/lib/taskKindLabels";

export default function SpeakingTaskKindsPage() {
  const params = useParams();
  const slug = typeof params.topicSlug === "string" ? params.topicSlug : "";
  const topic = topicFromSlug(slug);
  const kinds = topic ? getTaskKindsForTopic("speaking", slug) : [];
    ...
    <>
      ...
      <main className="max-w-2xl mx-auto px-6 py-10">
        ...
        {!topic ? (
          ...
          <>
            <p className="text-gray-500 mb-6">Topic: <span className="font-medium text-gray-800">{topic}</span></p>
            <ul className="flex flex-col gap-2">
              {kinds.map((kind) => (
                <li key={kind}>
                  <Link
                    href={`/practice/speaking/topics/${topicSlug(topic)}/tasks/${kind}/questions`}
                    className="block rounded-xl border bg-white px-4 py-3 text-gray-800 font-medium capitalize hover:border-blue-300 hover:bg-blue-50/50 transition-colors"
                  >
                    {speakingTaskKindLabel(kind)}
                  </Link>
                </li>
              ))}
            </ul>
