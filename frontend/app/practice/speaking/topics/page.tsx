...
export default function SpeakingTopicsPage() {
  const topics = getTopicsForSkill("speaking");
  ...
  return (
    <>
      ...
      <main className="max-w-2xl mx-auto px-6 py-10">
        ...
        <p className="text-gray-500 mb-6">Choose a topic for speaking practice.</p>
        <ul className="flex flex-col gap-2">
          {topics.map((topic) => (
            <li key={topic}>
              <Link
                href={`/practice/speaking/topics/${topicSlug(topic)}/tasks`}
                className="block rounded-xl border bg-white px-4 py-3 text-gray-800 font-medium hover:border-blue-300 hover:bg-blue-50/50 transition-colors"
              >
                {topic}
              </Link>
            </li>
